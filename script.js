// Simple side-scrolling platformer with clean rendering
...
const platforms=[
  ...
  {x:720,y:200,w:150,h:15,c:'#8B4513'},
  {x:940,y:320,w:180,h:15,c:'#8B4513'},
  {x:1180,y:380,w:200,h:15,c:'#8B4513'},
  {x:1450,y:300,w:150,h:15,c:'#8B4513'},
  {x:1700,y:420,w:180,h:15,c:'#8B4513'}
];
...
function updateEnemies(){
  enemies.forEach(e=>{e.x+=e.speed*e.dir; if(e.x<0||e.x+e.width>WORLD_WIDTH) e.dir*=-1; e.y=ground.y-e.height;});
}
...
function draw(){
  ...
  ctx.fillStyle=ground.color; ctx.fillRect(0-cameraX,ground.y,WORLD_WIDTH,ground.height);
  // platforms
  ...
  // enemies
  updateEnemies();
  enemies.forEach(e=>{ctx.fillStyle=e.color; ctx.fillRect(e.x-cameraX,e.y,e.width,e.height);});
  // collisions
  checkCollisions();
  // HUD
