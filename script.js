// Simple side‑scrolling platformer with sky background
...
function applyGravity(){
  ...
  platforms.forEach(p=>{const onTop=player.y+player.height>p.y&&player.y+player.height-player.vy<=p.y&&player.x+player.width>p.x&&player.x<p.x+p.width;if(onTop){player.y=p.y-player.height;player.vy=0;player.canJump=true;}});
...
function draw(){
  ...
  const drawRect=(obj,color)=>{ctx.fillStyle=color;ctx.fillRect(obj.x-cameraX,obj.y,obj.width,obj.height);};
